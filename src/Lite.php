...
use PhalApi\Restful\RestfulRequest;

use PhalApi\Restful\Handler;
use PhalApi\Restful\Handler\ErrorHandler;
    ...
    public function dispatch(Handler $errorHandler = null)
    {
        ...
        switch ($routeInfo[0]) {
            case \FastRoute\Dispatcher::NOT_FOUND:
                // ... 404 Not Found (兼容无路由)
                if (!(strpos($_SERVER['REQUEST_URI'], 's=' . (isset($routeInfo[1]) ? $routeInfo[1] : '')) !== FALSE)) {
                    $response = \PhalApi\DI()->response;
                    $response->setRet(404);
                    $response->setMsg(\PhalApi\T('Method Not Found'));

                    // ... 404 Method Not Allowed (异常处理)
                    $errorHandler->execute($response);
                }
            ...
            case \FastRoute\Dispatcher::METHOD_NOT_ALLOWED:
                ...
                $response = \PhalApi\DI()->response;
                $response->setRet(405);
                $response->setMsg(\PhalApi\T('Method Not Allowed, It Should be: {methods}',
            ...
            case \FastRoute\Dispatcher::FOUND:
                ...
                \PhalApi\DI()->request = new RestfulRequest($vars);
                break;
        }
    }
}
